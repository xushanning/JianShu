...
import com.xu.module.wan.base.LoadMoreAdapter
...
class HomeFragment(
    ...
    private val collectViewModel by viewModels<ArticleCollectViewModel>()
    ...
    override fun initView(mDataBinding: WFragmentHomeBinding) {
        //这里有个坑，设置footer必须这样设置，因为withLoadStateFooter返回一个ConcatAdapter对象，不能在run里面设置
        rv_home.run {
            adapter = pagingAdapter.withLoadStateFooter(LoadMoreAdapter {
                pagingAdapter.retry()
            })
